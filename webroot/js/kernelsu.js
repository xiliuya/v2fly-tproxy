...
function spawn(command, args = [], options = {}) {
  const child = {
    ...
    emit(event, ...args$1) {
      if (this.listeners[event]) {
        this.listeners[event].forEach((listener) => listener(...args$1));
      }
    },
  };
...
function fullScreen(isFullScreen) {
  if (typeof ksu !== "undefined") ksu.fullScreen(isFullScreen);
}
/**
...
function toast(message) {
  if (typeof ksu !== "undefined") ksu.toast(message);
 ...
 */
async function listPackages(type = "all") {
  const pkgType = {
  ...
  if (typeof globalThis.$packageManager?.getInstalledPackages === "function") {
    const pmArgs = {
      all: [4202496, 0], // [flags, userId]
      user: [0, 0], // [flags, userId]
    ...
    try {
      const rawResult = await globalThis.$packageManager.getInstalledPackages(
        ...pmArgs[type],
      );
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!(type in pmArgs)) {
      return reject(
        /* @__PURE__ */ new Error(`Unknown listPackages type: ${type}`),
      );
    }
    let pkgs = [];
    let stderr = "";
    ...
    pm.stdout.on("data", (data) => {
      if (data.trim() !== "") pkgs.push(data.trim().replace(/^package:/, ""));
    });
    ...
    pm.on("exit", (code) => {
      if (code !== 0) {
        return reject(
          /* @__PURE__ */ new Error(
            `pm process exited with code ${code}: ${stderr.trim()}`,
          ),
        );
      }
      resolve(pkgs);
...
function getPackagesInfo(pkg) {
  return new Promise((resolve, reject) => {
    if (!pkg) return resolve([]);

    if (typeof globalThis.ksu?.getPackagesInfo !== "function") {
      return reject(
        /* @__PURE__ */ new Error("ksu.getPackagesInfo is not available."),
      );
    }
    ...
    try {
      ...
      if (!Array.isArray(pkg) && result.length === 1) resolve(result[0]);
      else resolve(result);
    } catch (error) {
      reject(
        /* @__PURE__ */ new Error(
          `Failed to get package info: ${error.message}`,
        ),
      );
    }
  });
 ...
 *   - [{},{}]
 */
function getPackagesIcon(pkg, size = 100) {
  return new Promise((resolve, reject) => {
    if (!pkg) return resolve([]);
    if (typeof globalThis.ksu?.getPackagesIcons !== "function") {
      return reject(
        /* @__PURE__ */ new Error("ksu.getPackagesIcons is not available."),
      );
    }
    const pkgs = Array.isArray(pkg) ? pkg : [pkg];
    if (pkgs.length === 0) return resolve([]);
    try {
      ...
      reject(
        /* @__PURE__ */ new Error(
          `Failed to get package icon: ${error.message}`,
        ),
      );
    }
  });
